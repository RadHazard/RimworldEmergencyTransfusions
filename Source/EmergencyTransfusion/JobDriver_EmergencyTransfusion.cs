...
namespace EmergencyTransfusion
{
    ...
    public class JobDriver_EmergencyTransfusion : JobDriver
    {
        ...
        {
            // var f = this;
            // f.FailOnDespawnedNullOrForbidden<JobDriver_TendPatient>(TargetIndex.A);
        ...
        public override void Notify_DamageTaken(DamageInfo dinfo)
        {
            ...
            if (!dinfo.Def.ExternalViolenceFor((Thing)pawn) || pawn.Faction == Faction.OfPlayer || pawn != Patient)
                return;
            pawn.jobs.CheckForJobOverride();
        }
        
        public static List<Toil> CollectMedicineToils(
            Pawn doctor,
            Pawn patient,
            Job job,
            Toil gotoToil,
        ...
        public static Toil FindMoreMedicineToil(
            Pawn doctor,
            Pawn patient,
            TargetIndex medicineIndex,
            Job job,
            Toil reserveMedicine) 
        {
            Toil moreMedicineToil = ToilMaker.MakeToil(nameof(FindMoreMedicineToil));
            moreMedicineToil.initAction = (Action)(() =>
            {
                if (!job.GetTarget(medicineIndex).Thing.DestroyedOrNull())
                    return;
                Thing bestMedicine = HealthAIUtility.FindBestMedicine(doctor, patient);
                if (bestMedicine == null)
                    return;
                job.SetTarget(medicineIndex, (LocalTargetInfo)bestMedicine);
                doctor.jobs.curDriver.JumpToToil(reserveMedicine);
            });
            return moreMedicineToil;
        }
    }
}